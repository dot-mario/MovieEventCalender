...
def get_lottecinema_moviesadagu():
    ...
    with create_retry_session() as session:
        try:
            ...
            match = re.search(
                r'EventID"\s*:\s*"(\d+)",\s*"MainEventID"\s*:\s*"(\d+)"',
                res.text,
                re.IGNORECASE,
            )
            if match:
                ...
                match_url = re.search(
                    r"EventTemplateSpeedMulti\?eventId=(\d+)", res.text, re.IGNORECASE
                )
                if match_url:
                    main_event_id = match_url.group(1)

            if not main_event_id:
                raise RuntimeError("메인 페이지에서 이벤트 ID를 찾을 수 없습니다.")
            ...
            multipart_payload = {"paramList": (None, json.dumps(inner_payload))}
            ...
            # 4. 데이터 파싱
            if "SpeedEventDetail" not in data:
                raise RuntimeError("API 응답에 SpeedEventDetail 필드가 없습니다.")

            if data["SpeedEventDetail"]:
                ...
                print("[Lotte] 현재 진행 중인 무비싸다구 이벤트가 없습니다.")

        except Exception as error:
            print(f"[Lotte] 무비싸다구 크롤링 중 오류 발생: {error}")
